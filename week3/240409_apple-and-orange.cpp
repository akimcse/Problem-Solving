...
//중복을 셀 필요는 없다. 같은 위치로 다른 과일이 떨어질 수도 있기 때문.
// 단순히 s나 t보다 크거나 작기만 하는 게 아니라 두 범위 사이에 들어와야 한다.
...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    for(int i=0; i<(int)oranges.size(); i++){
        if(b + oranges[i] <= t && b + oranges[i] >= s) orange_num++;
    }
    
    cout << apple_num << endl << orange_num;
}
